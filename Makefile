...
setup-local-cluster: ## Setup Minikube cluster with Prometheus and Grafana
	...
	@helm repo update
	@helm install prometheus bitnami/kube-prometheus --namespace default --set prometheus.service.type=ClusterIP --wait --timeout=5m || echo "Prometheus already installed"
	@helm install grafana bitnami/grafana --namespace default --set service.type=ClusterIP --set admin.user=admin --set admin.password=admin --wait --timeout=5m || echo "Grafana already installed"
